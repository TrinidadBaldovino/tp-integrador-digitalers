...
function leerDatosProducto(producto) {
  const precioConSimbolo = producto.querySelector('.precio').textContent // $1800
  const precioSinSimbolo = precioConSimbolo.slice(1) // 1800 -> string
  const precioNumero = Number(precioSinSimbolo) // 1800 -> number
...
export function procesarCarrito(e) {
  ...
  productosLS.forEach(function (producto) {
    ...
    div.innerHTML = `
                  <div class="col-4 mb-1">
                    ...
                    <div class="col-2">
                    <input type="number" min="1" class="form-control text-center cantidad" placeholder="Cantidad" value="${
                      producto.cantidad
                    }" >
  ...
  if (e.target.classList.contains('cantidad')) {
    ...
    productosLS.forEach(function (productoLs, index) {
      if (productoLs.id === id) {
        ...
        // debugger
        let total = Number(productoLs.cantidad) * parseInt(productoLs.precio)
